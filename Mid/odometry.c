...
void get_crntCoordinate(struct coordinate* crnt_coordinate){
    ...
    if(delta_LL > 0.0 && delta_LR < 0.0){ //時計回りに旋回している
        ...
        delta_L = 0.0;
        // 旋回量の計算
        delta_rad_e = -1 * (abs(delta_LL) + abs(delta_LR)) / (float)wheel_dist;
    ...
    delta_theta_e = (float)((delta_rad_e * 180.0) / PI_FLOAT);
    ...
    float delta_rad = delta_rad_g;      // 旋回量計算に使用するデバイスの選択
    float delta_theta = delta_theta_g;  // 旋回量計算に使用するデバイスの選択
