...
void get_crntCoordinate(struct coordinate* crnt_coordinate){
    ...
    /* 左右モータの回転量を計算する[rad] */
    float delta_PhL = PI_FLOAT * (float)(motor_get_counts(left_motor)  - motor_get_pre_counts(left_motor))  / 180.0f;
    float delta_PhR = PI_FLOAT * (float)(motor_get_counts(right_motor) - motor_get_pre_counts(right_motor)) / 180.0f;
    ...
    float delta_rad_e = 0.0f;
    float delta_theta_e = 0.0f;
    ...
    if(delta_LL > 0.0f && delta_LR < 0.0f){ //時計回りに旋回している
    ...
    else if((delta_LL < 0.0f && delta_LR > 0.0f)){ // 反時計回りに旋回している
        //ロボットの移動距離
        delta_L = 0.0f;
        // 旋回量の計算
        delta_rad_e = -1.0f * (abs(delta_LL) + abs(delta_LR)) / (float)wheel_dist;
        ...
        if(abs(delta_rad_e) > 0.174){ //delta_rad_eが十分大きい
            delta_L = 2.0f * (delta_L / delta_rad_e) * (float)sin((double)(delta_rad_e / 2.0f));
        }
        
    }
    delta_theta_e = (float)((delta_rad_e * 180.0f) / PI_FLOAT);
    ...
    crnt_coordinate->x      = pre_coordinate.x + (float)( (double)delta_L * cos((double)(pre_rad + (delta_rad / 2.0))) );
    crnt_coordinate->y      = pre_coordinate.y + (float)( (double)delta_L * sin((double)(pre_rad + (delta_rad / 2.0))) );
    crnt_coordinate->theta  = pre_coordinate.theta + delta_theta;
