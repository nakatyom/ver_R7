...
#define TIRE_SIZE_R     90.0f   // 右タイヤ直径[mm]
#define TIRE_SIZE_L     90.0f   // 左タイヤ直径[mm]
#define TRED_SIZE       140.0f  // トレッド直径[mm]
...
void mid_velocity_control(float velo_str_tgt, float velo_rot_tgt){
    ...
    int velo_r_tmp  = (cnt_r - cnt_r_pre) * SAMPLE_RATE;    // cnt ⇒ deg/sec(motor)
    int velo_l_tmp  = (cnt_l - cnt_l_pre) * SAMPLE_RATE;    // cnt ⇒ deg/sec(motor)             
    
    float velo_r    = ((float)velo_r_tmp / 360.0f) * (3.141592f * TIRE_SIZE_R);   // deg/sec(motor) ⇒ mm/sec(motor) 
    float velo_l    = ((float)velo_l_tmp / 360.0f) * (3.141592f * TIRE_SIZE_L);   // deg/sec(motor) ⇒ mm/sec(motor)
...
//    float velo_rot_u = 0;   // debug
    ...
    /* 速度操作量計算（FF項とFB項の合成）*/
    float mot_r_u_str = ((velo_str_tgt + velo_str_u) / (3.141592f * TIRE_SIZE_R)) * 360.0f; // mm/sec(robot) ⇒ deg/sec(motor)
    float mot_l_u_str = ((velo_str_tgt + velo_str_u) / (3.141592f * TIRE_SIZE_L)) * 360.0f; // mm/sec(robot) ⇒ deg/sec(motor)
